...
The following section will present an overview of the project. The design of a few key parts of the project has not yet been finalized, and although a placeholder solution is provided, a proper solution will be presented in the future.

This distributed peer-to-peer filesystem uses a layered architecture. At the lowest level is the Chord protocol and algorithm for mapping keys to nodes. On top of Chord sits an implementation of a peer-to-peer distributed hash table (DHT). This DHT is used by the FUSE userspace file system interface to retrieve, update and delete files.

Currently there is no clear consensus in the literature on what distributed file system components lead to achieving the best possible performance (why?) 

\subsection{Preparation}

As the filesystem will be based on a distributed peer-to-peer model, the user's files may end up bein
