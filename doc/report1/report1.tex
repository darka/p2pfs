...
\title{MInf4 Project Phase 1 Report\\
Distributed Peer-to-peer Filesystem}  

\author{Darius Scerbavicius\\s0969755\\
...
This distributed peer-to-peer filesystem uses a layered architecture (Figure~\ref{layeredarch}).

\begin{figure}
\begin{center}
  \includegraphics[width=11cm]{archgraph.png}
\end{center}
\caption{Distributed peer-to-peer filesystem architecture}
\label{layeredarch}
\end{figure}  

At the lowest level, the TCP protocol \cite{tcp} will be used for communication among the nodes participating in the peer-to-peer network. TCP was chosen due to its reliable stream delivery service. The very low level details of using the protocol will, however, not be presented in this report.

On top of the TCP protocol, the Chord protocol and algorithm \cite{chord} will be used for mapping files deployed on the filesystem to nodes, selected by the algorithm to store the files. This will be further explained in the Key-Based Routing section.

On top of Chord, an implementation of a peer-to-peer distributed hash table will be provided. Alongside Chord, for each client using the network, this DHT will maintain a data cache on the client's machine, containing the files that belong to that client, as well as files that other nodes in the network intend to store on that particular node. 

A virtual filesystem will then provide an interface to the DHT and the data cache using the FUSE \cite{fuse} library, allowing the filesystem to be mounted in userspace on any machine running Linux. 
...
A KBR protocol can route a message with any given key, and the definition of a key is imposed by the service using the KBR protocol. In our case, the key will be the SHA-1 hash of the file path, and the node identifier will be the SHA-1 hash of the IP address of the node. In order to be able to route the message to the closest node, the distance between keys and identifiers has to be defined. The distance between the node's identifier and the key of the message will be determined by taking the difference between them. %(talk about chord's routing algorithm like in CFS paper)

This report will not go into a detailed discussion about the workings of the Chord protocol, which is described in \cite{chord}, but a brief outline is provided.
...
The DHT will use Chord to determine which node the key value pair should be stored on. To ensure data durability in the network, replicas of the value will need to be stored on a number of nodes in the Chord ring, after the successor of the node presented by Chord. This is discussed in Future Plans.

\item retrieve(key)

The DHT will first check whether the client itself currently has an object with the key in its data cache. In the case that it does, the DHT will return its object descriptor, namely the Unix file descriptor.
...
Once the file is downloaded to the client's data cache, a file descriptor will be returned.
...
For each user of the distributed peer-to-peer network, an index file, containing the metadata of files (such as the file's owner, group, and permissions) belonging to the particular user, will be stored in the network.
...
\end{itemize}
The contents of these three tables will contain file names, paths and file system attributes. This data will be used by FUSE \cite{fuse} (see Frontend) in order to answer queries about the attributes of all file system objects.
...
The goal of implementing a security layer for the filesystem is to ensure that no filesystem user can read any other users' files, as well as to prevent malicious users from overwriting other user data.
...
% introduce another SQL database here?
...
The symmetric key mechanism will ensure that the user of the current node cannot read the contents of files that belong to other users of the system, as the current user will only posses own AES key, and not the key of any other nodes in the network.
...
As the goal of the project is to produce an implementation of a distributed peer-to-peer filesystem targeting the real world, a prototype of the system described will be made available as an open source project under the GPL \cite{gpl} license. As FUSE will be used to implement the frontend of the file system, the implementation is expected to run on any Linux-based operating system, as well as Mac OS X.
...
\subsubsection{ns-3}
The first method considered, was to target ns-3 for the test bed environment, as it has previously been used for this purpose when testing the Infinit filesystem \cite{towards}. This method was attempted at first, but it soon became evident that it has a drawback: the filesystem implementation used for simulation would differ too much from the implementation targeting the real world. %This was the case for the Infinit project, as the implementation used in the simulated environment produced  
...
The Python application used in the experiment is currently being developed into an implementation of Chord.
...
First of all, there is still no clear plan on how to ensure data durability, as many nodes in the network will often go off-line. The current plan is to rely on data redundancy and store file replicas on a number of nodes in the Chord ring after the successor of the node which was returned by a query. 
...
It should also be noted that a suitable number of nodes carrying the replicas has not yet been determined, and a batch of experiments will need to be executed to find a suitable value. These experiments have also not yet been designed.
...
\hline
Address the design issues listed in future plans & June 1st 2013 - June 30th 2013 \\ 
\hline
Finish implementing Chord & June 1st 2013 - August 30th 2013 \\ 
\hline
\hline
{\bf Milestone: Chord layer implemented} & September 1st 2013 \\
\hline
\hline
Implement DHash and the simple security scheme on top of the DHash layer & September 1st 2013 - September 30th 2013 \\
\hline
Implement the FUSE interface to the filesystem. & October 1st 2013 - October 30th 2013 \\
\hline
Determine if there are any missing parts in the filesystem that prevent the Andrew benchmark from running, and implement them & November 1st 2013 - November 15th 2013 \\
\hline
\hline
{\bf Milestone: Andrew benchmark running} & 15th November 2013 \\
\hline
\hline
Compare Andrew benchmark results to other similar filesystems, determine if there are any shortcomings and devise a plan on how to overcome them & November 15th 2013 - December 1st 2013 \\
\hline
Address the determined shortcomings of the filesystem & 1st December 2013 - 15th January 2014 \\
\hline
Finish evaluating the filesystem and obtain final benchmark results & 15th January 2014 - 30th January 2014 \\
\hline
\hline
...
\bibitem{tcp}
Transmission Control Protocol. Protocol Specification. http://tools.ietf.org/html/rfc793
\bibitem{chord} 
